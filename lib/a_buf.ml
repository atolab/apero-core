...
let clear buf = {buf with rd_idx = 0; wr_idx = 0;}

let expand n buf = 
  let nbuffer = Bigstringaf.create (n + capacity buf) in 
  Bigstringaf.blit buf.buffer ~src_off:0 nbuffer ~dst_off:0 ~len:buf.wr_idx;
  {buf with buffer = nbuffer}

let reader_index buf = buf.rd_idx

let set_reader_index i buf = 
  if i >= 0 && i <= buf.wr_idx
  then return {buf with rd_idx = i}
  else fail (`OutOfBounds (`Msg (
    Printf.sprintf "A_buf.set_reader_index with %d out of (0 .. %d)" i buf.wr_idx)))

let mark_reader_index buf = {buf with rd_mark = buf.rd_idx}

let reset_reader_index buf = {buf with rd_idx = buf.rd_mark}

let writer_index buf = buf.wr_idx

let set_writer_index i buf = 
  if i >= buf.rd_idx && i <= capacity buf
  then return {buf with wr_idx = i}
  else fail (`OutOfBounds (`Msg (
    Printf.sprintf "A_buf.set_writer_index with %d out of (%d .. %d)" i buf.rd_idx (capacity buf))))

let mark_writer_index buf = {buf with wr_mark = buf.wr_idx}

let reset_writer_index buf = {buf with wr_idx = buf.wr_mark}


let readable_bytes buf = (writer_index buf) - (reader_index buf)

let readable buf = readable_bytes buf > 0

let writable_bytes buf = (capacity buf) - (writer_index buf)

let writable buf = writable_bytes buf > 0


let skip n buf = set_writer_index (buf.rd_idx + n) buf |> function
  | Ok buf -> return buf
...
let copy_to ~src ~src_idx ~dst ~dst_idx ~len = 
  if src_idx >= 0 && len >= 0 && src_idx + len <= src.wr_idx then
    begin
...
let read_char buf =
  if readable buf then
    begin
      let c = Bigstringaf.get buf.buffer buf.rd_idx in
      return (c, {buf with rd_idx = buf.rd_idx+1})
    end
  else 
...
let read_chars len buf = 
  if len >= 0 && len <= readable_bytes buf then
    begin
      ...
      Bigstringaf.blit_to_bytes buf.buffer ~src_off:buf.rd_idx s ~dst_off:0 ~len;
      return (Bytes.to_string s, {buf with rd_idx = buf.rd_idx + len})
    end 
  else 
...
let read_bytes len buf = 
  if len >= 0 && len <= readable_bytes buf then
    begin
      ...
      Bigstringaf.blit buf.buffer ~src_off:buf.rd_idx dst ~dst_off:0 ~len ;
      return (dst, {buf with rd_idx = buf.rd_idx + len})
    end
  else 
...
let get_chars ~at len buf = 
  if at >= 0 && len >= 0 && at + len <= buf.wr_idx then
    begin
      let s = Bytes.create len in
...
let rec write_char c buf = 
  if writable buf then
    begin
      Bigstringaf.set buf.buffer buf.wr_idx c;
      return { buf with wr_idx = buf.wr_idx + 1}
    end
  else
    match buf.grow with 
...
let rec write_chars s buf =     
  ...
  if len <= writable_bytes buf then
    begin
      Bigstringaf.blit_from_bytes (Bytes.of_string s) ~src_off:0 buf.buffer ~dst_off:buf.wr_idx ~len;
      return {buf with wr_idx = buf.wr_idx + len}
    end 
  else 
    match buf.grow with 
...
let rec write_bytes src buf  =
  ...
  if len <= writable_bytes buf then
    begin
      Bigstringaf.blit src ~src_off:0 buf.buffer ~dst_off:buf.wr_idx ~len ;
      return {buf with wr_idx = buf.wr_idx + len}
    end
  else
    match buf.grow with 
...
let rec write_buf src buf  =
  ...
  if len <= writable_bytes buf then
    begin
      Bigstringaf.blit src.buffer ~src_off:src.rd_idx buf.buffer ~dst_off:buf.wr_idx ~len ;
      return {buf with wr_idx = buf.wr_idx + len}
    end
  else
    match buf.grow with 
...
let hexdump ?separator:(sep="") buf =
  let rec hexdump buf idx =
    if idx < buf.wr_idx then 
    (Printf.sprintf "%02x%s" (int_of_char @@ Bigstringaf.get buf.buffer idx) sep ) ^ (hexdump buf (idx+1))
    else "" in 
  hexdump buf 0
    
let to_string buf =
  "(rd_idx: " ^ (string_of_int buf.rd_idx) ^ ", wr_idx: " ^ (string_of_int buf.wr_idx) ^ " content: " ^ (hexdump buf ~separator:":")
